...
steps:
  ...
  - name: Prefetch untranslated German strings
    ...
    run: |
      ...
      npx tsx scripts/i18n/show-untranslated.ts de > /tmp/gh-aw/data/untranslated-de.txt
...
Activation: this Sunday (or manual) run, after Node dependencies are installed. Required evidence is `/tmp/gh-aw/data/untranslated-de.txt` (every remaining `{id}: {english}` line) and `/tmp/gh-aw/data/untranslated-count.txt`. Follow the `i18n-frontend-translate` skill. Locale is `de`.

Required effects when the prefetch file has at least one line:

1. Translate or ignore **every** id listed in `untranslated-de.txt` (full backlog for this run).
...
6. Open one pull request via `create_pull_request` (reviewer `znarf` is assigned automatically). Title starts with `[i18n-de] `. Body: prefetch count, translated vs IGNORED counts, notable IGNORED ids, and how many untranslated ids remain (re-run `show-untranslated.ts de` if needed).
