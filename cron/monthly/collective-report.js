...
const init = () => {
  ...
  models.Collective.findAll(query)
    .tap(collectives => {
      console.log(
        `Preparing the ${month} report for ${collectives.length} collectives`,
      );
    })
...
const getTopBackers = (startDate, endDate, tags) => {
  ...
  if (topBackersCache[cacheKey])
    ...
    return models.Collective.getTopBackers(startDate, endDate, tags, 5)
      .then(backers => {
        if (!backers) return [];
        return Promise.map(backers, backer =>
          processBacker(backer, startDate, endDate, tags),
        );
      })
      .then(backers => {
...
const generateDonationsString = (backer, transactions) => {
  if (!backer.name) {
    debug(
      `Skipping ${backer.username} because it doesn't have a name (${
        backer.name
      })`,
    );
    return;
  ...
  for (let i = 0; i < Math.min(3, transactions.length); i++) {
    ...
    donationsHTMLArray.push(
      ...
      )} to <a href="https://opencollective.com/${
    ...
    donationsTextArray.push(
      `${formatCurrency(
        transaction.amount,
        transaction.currency,
      )} to https://opencollective.com/${transaction.collective.slug}`,
    );
  }
...
const processCollective = collective => {
  ...
  let emailData = {};
  const options = {};
  const csv_filename = `${moment(d).format(dateFormat)}-${
    collective.slug
  }-transactions.csv`;
  ...
  return Promise.all(promises)
    .then(results => {
      ...
      data.topBackers = _.filter(
        results[0],
        backer => backer.donationsString.text.indexOf(collective.slug) === -1,
      ); // we omit own backers
      return getTiersStats(results[1], startDate, endDate).then(res => {
        data.collective = _.pick(collective, [
          'id',
          'name',
          'slug',
          'currency',
        ...
        if (
          data.collective.transactions &&
          data.collective.transactions.length > 0
        ) {
          const collectivesById = { [collective.id]: collective };
          const csv = models.Transaction.exportCSV(
            data.collective.transactions,
            collectivesById,
          );
