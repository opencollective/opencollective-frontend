...
const init = () => {
  models.Member.findAll({
    ...
    .tap(transactionsGroups => {
      console.log(
        `${
          transactionsGroups.length
        } different collectives got new backers since ${TenMinutesAgo}`,
      );
    })
...
const notifyCollective = async (CollectiveId, milestone, collective) => {
  ...
  if (!twitterAccount) {
    debug(
      `${
        collective.slug
      }: the collective id ${CollectiveId} doesn't have a twitter account connected, skipping`,
    );
...
const processNewMembersCount = async newMembersCount => {
  ...
  if (backersCount < 10) {
    debug(
      `${collective.slug} only has ${backersCount} ${pluralize(
        'backer',
        backersCount,
      )}, skipping`,
    );
  ...
  if (hasPassedMilestone(1000)) {
    console.log(
      `🎉 ${
        collective.slug
      } just passed the 1,000 backers milestone with ${backersCount} backers`,
    );
  ...
  if (hasPassedMilestone(100)) {
    console.log(
      `🎉 ${
        collective.slug
      } just passed the 100 backers milestone with ${backersCount} backers`,
    );
...
const compileTwitterHandles = (userCollectives, total, limit) => {
  const twitterHandles = userCollectives
...
const compileTweet = async (collective, template, twitterAccount) => {
  ...
  if (template === 'tenBackers') {
    ...
    replacements.topBackersTwitterHandles = compileTwitterHandles(
      backers,
      10,
      10,
    );
  ...
  if (process.env.NODE_ENV === 'production') {
    try {
      ...
      const in_reply_to_status_id = get(
        twitterAccount,
        'settings.milestones.lastTweetId',
      );
      ...
      set(
        twitterAccount,
        'settings.milestones.tweetSentAt',
        new Date(res.created_at),
      );
      set(twitterAccount, `settings.${template}.tweetId`, res.id_str);
      set(
        twitterAccount,
        `settings.${template}.tweetSentAt`,
        new Date(res.created_at),
      );
      ...
      res.url = `https://twitter.com/${res.user.screen_name}/status/${
        res.id_str
      }`;
      return res;
    } catch (e) {
