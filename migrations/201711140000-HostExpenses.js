...
const addPaymentMethods = (sequelize) => {
  ...
  const addPaymentMethodForCollective = (collective) => {
    ...
    if (DRY_RUN) {
      console.log(">>> create payment method for ", collective.id, collective.slug, collective.type);
  ...
  return sequelize.query(`DELETE FROM "PaymentMethods" WHERE service='opencollective'`)
    .then(() => {
      return sequelize.query(`SELECT id, name, slug, currency, type FROM "Collectives" WHERE type='COLLECTIVE'`, { type: sequelize.QueryTypes.SELECT })
        .map(addPaymentMethodForCollective);
    })
    .then(() => {
      // get all hosts
      return sequelize.query(`SELECT c.id, c.name, c.slug, c.currency, c.type FROM "Collectives" c LEFT JOIN "ConnectedAccounts" ca ON ca."CollectiveId" = c.id WHERE c.type!='COLLECTIVE' AND ca.service='stripe' AND ca."deletedAt" IS NULL`, { type: sequelize.QueryTypes.SELECT })
        .map(addPaymentMethodForCollective);
    })
}
