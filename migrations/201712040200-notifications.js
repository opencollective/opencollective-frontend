...
const deleteNotifications = (sequelize) => {
  ...
  return sequelize.query(`DELETE FROM "Notifications" WHERE (type='user.yearlyreport' OR type='user.monthlyreport') AND active IS TRUE`);
}
...
const updateNotifications = (sequelize) => {
  ...
  return sequelize.query(`SELECT n.id, u."CollectiveId" FROM "Notifications" n LEFT JOIN "Users" u ON n."UserId"=u.id WHERE (type='user.yearlyreport' OR type='user.monthlyreport') AND active IS FALSE`, { type: sequelize.QueryTypes.SELECT }).then(notifications => Promise.map(notifications, updateNotification, { concurrency: 1 }));
}
