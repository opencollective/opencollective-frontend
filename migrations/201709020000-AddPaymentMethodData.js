...
const updatePaymentMethods = (sequelize) => {
  ...
  const getCollectiveById = (id) => {
    ...
    return sequelize.query(`SELECT id, currency FROM "Collectives" WHERE id=:id LIMIT 1`, { replacements: { id }}).then(res => {
  ...
  const updatePaymentMethod = (pm) => {
    const data = JSON.stringify(Object.assign({}, pick(pm, [ 'brand', 'country', 'funding', 'fullName', 'expMonth', 'expYear' ]), pm.data));
    
    return getCollectiveById(pm.CollectiveId)
      .then(collective => {
        ...
        // return null;
        return sequelize.query(`UPDATE "PaymentMethods" SET data=:data, currency=:currency WHERE id=:id`, { replacements: { data, id: pm.id, currency }});
      });
  };
...
module.exports = {
  up: function (queryInterface, DataTypes) {
    ...
    .then(() => queryInterface.renameColumn('PaymentMethods', 'identifier', 'name'))
  },
