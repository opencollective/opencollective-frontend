...
const insert = (sequelize, table, entry) => {
  ...
  if (DRY_RUN) {
    ...
    `, { replacements: entry }).catch(e => console.error("Error inserting", entry, e.name));
  }
}
...
const addNotifications = (sequelize) => {
  ...
  `, { type: sequelize.QueryTypes.SELECT }).map(insertNotification)
}

module.exports = {
  up: function (queryInterface, Sequelize) {
    return addNotifications(queryInterface.sequelize)
      .then(() => {
