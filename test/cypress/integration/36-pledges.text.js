describe('Pledges', () => {
  ...
  describe('creating an individual pledge', () => {
    it('Create the pledge', () => {
      ...
      cy.get('[data-cy="currencyAmount"]', { timeout: 12000 }).should('contain', '$');
    });

    it('join an existing pledge individual pledge', () => {
      cy.visit('/opencollective');
      let value1 = '';
      cy.get('[data-cy="currencyAmount"]', { timeout: 12000 }).then($val => {
        value1 = $val.text();
      });
    ...
    it('creating a pledge and ensuring the amount is visible loggedout', () => {
      ...
      cy.get('[id=publicMessage]').clear().type('publicMessage');
      cy.get('[data-cy="submit"]').click();
      cy.get('[data-cy="pledgeStats"]', { timeout: 12000 }).contains('$');
      let amountOutside = '';
      cy.get('[data-cy="currencyAmount"]', { timeout: 12000 })
        .should('contain', '$')
        .then($val => {
      ...
      cy.get('[data-cy="amountPledgedTotal"] > span', { timeout: 12000 }).should('contain', amountOutside);
    });
  });
  it('creating a pledge unable to verify the organization', () => {
    ...
    cy.get('[data-cy="errorMessage"]', { timeout: 12000 }).should(
      'contain',
  ...
  it('creating a pledge unable to verify the repository', () => {
    ...
    cy.get('[data-cy=githubHandleInput]').clear().type('demo/dummy');
    cy.get('[data-cy=publicMessage]').clear().type('publicMessage');
    cy.get('[data-cy="errorMessage"]', { timeout: 12000 }).contains(
...
describe('check FAQ context in each pledge is valid or not', () => {
  it('verift what is pledge ?', () => {
    ...
    cy.get('[data-cy="whatIsAPledge"]', { timeout: 12000 }).should(
      'contain',
  ...
  it('verift how do i claim a pledge ?', () => {
    ...
    cy.get('[data-cy="howDoIClaimPledge"]', { timeout: 12000 }).should(
      'contain',
