...
describe('webhooks.stripe.creditcard.test.js', () => {
  describe('when it receives a subscription that does not match with our plan', () => {
    ...
    beforeEach(() => {
      ...
      stripeMock.webhook_payment_succeeded.data.object.lines.data[0].plan.id =
        'some-foreign-plan-type';

      sandbox = sinon.createSandbox();
      sandbox
        .stub(appStripe.events, 'retrieve')
        .callsFake(() => Promise.resolve(stripeMock.webhook_payment_succeeded));
    });
