...
export const unsubscribe = (req, res, next) => {
  ...
  const identifier = `${email}.${slug || 'any'}.${type}.${
...
const sendEmailToList = (to, email) => {
  ...
  email.from =
    email.from ||
    `${collectiveSlug} collective <hello@${collectiveSlug}.opencollective.com>`;
    ...
    .tap(subscribers => {
      if (subscribers.length === 0)
        throw new errors.NotFound(
          `No subscribers found in ${collectiveSlug} for email type ${type}`,
        );
    })
    .then(results => results.map(r => r.email))
    .then(recipients => {
      debugEmail(
        `Sending email from ${email.from} to ${to} (${
          recipients.length
        } recipient(s))`,
      );
...
export const approve = (req, res, next) => {
  ...
  return request
    .get(
      `https://${mailserver}.api.mailgun.net/v3/domains/opencollective.com/messages/${messageId}`,
      requestOptions,
    )
    .then(json => {
      email = json;
    ...
    .then(() =>
      res.send(
        `Email from ${email.sender} with subject "${
          email.Subject
        }" approved for the ${email.To} mailing list`,
      ),
    )
    .catch(e => {
      if (e.statusCode === 404)
        return next(
          new errors.NotFound(
            `Message ${messageId} not found on the ${mailserver} server`,
          ),
        );
      else return next(e);
    });
...
export const getNotificationType = email => {
  ...
  if (
    ['info', 'hello', 'members', 'admins', 'admins'].indexOf(mailinglist) !== -1
  ) {
    mailinglist = 'admins';
  }
...
export const webhook = (req, res, next) => {
  ...
  debugWebhook(
    `email received for ${mailinglist} mailinglist of ${collectiveSlug}`,
  );
  ...
  if (body.indexOf('<!-- OpenCollective.com -->') !== -1) {
    debugWebhook(
      `Email from ${email.from} with subject ${
        email.subject
  ...
  models.Collective.find({ where: { slug: collectiveSlug } })
    ...
    .then(collective =>
      models.Notification.getSubscribersCollectives(
        collective.slug,
        mailinglist,
      ),
    )
    .tap(results => {
      ...
      subscribers = results.map(s => {
        if (s.image) {
          ...
          s.roundedAvatar = `https://ui-avatars.com/api/?name=${encodeURIComponent(
            s.name,
    ...
    .then(admins => {
      const messageId = email['message-url'].substr(
        email['message-url'].lastIndexOf('/') + 1,
      );
      const mailserver = email['message-url'].substring(
        8,
        email['message-url'].indexOf('.'),
      );
      const getData = user => {
        return {
          from: email.from,
      ...
      return Promise.map(admins, admin =>
        emailLib.send(
          'email.approve',
          `admins@${collectiveSlug}.opencollective.com`,
          getData(admin),
          { bcc: admin.email },
        ),
      );
    ...
    .catch(e => {
      switch (e.message) {
        ...
        case 'no_admins':
          return res.send({
            error: {
              message: `There is no admins to approve emails sent to ${
                email.recipient
              }`,
            },
          });
