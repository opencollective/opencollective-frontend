...
export default app => {
  ...
  app.post(
    '/forest/actions/activate-subscription',
    ...
    (req, res) => {
      ...
      models.Subscription.findOne({ where: { id } })
        ...
        .then(() => {
          res
            .status(200)
            .send({ success: 'The subscription was successfully activated.' });
        })
        .catch(e => {
  ...
  app.post(
    '/forest/actions/cancel-subscription',
    Liana.ensureAuthenticated,
    (req, res) => {
      const data = req.body.data;
      const id = data.attributes.ids[0];
      models.Subscription.findOne({ where: { id } })
        ...
        .then(() => {
          res
            .status(200)
            .send({ success: 'The subscription was successfully canceled.' });
        })
        .catch(e => {
          res.status(400).send({ error: e.message });
        });
  ...
  app.post(
    '/forest/actions/delete-collective-and-dependencies',
    Liana.ensureAuthenticated,
    async (req, res) => {
      const data = req.body.data;
      const id = data.attributes.ids[0];
      models.Collective.findOne({ where: { id } })
        ...
        .then(() => {
          const msg =
            'The collective and its dependencies were successfully deleted.';
          res.status(200).send({
            html: `<p>${msg}</p>
  ...
  app.post(
    '/forest/actions/delete-user-and-dependencies',
    Liana.ensureAuthenticated,
    async (req, res) => {
      const data = req.body.data;
      const id = data.attributes.ids[0];
      try {
        ...
        // Delete Expenses
        await models.Expense.findAll({ where: { UserId: user.id } }).then(
          expenses => {
            expenses.forEach(async expense => {
              await expense.destroy();
            });
  ...
  app.post(
    '/forest/actions/delete-user-and-merge',
    Liana.ensureAuthenticated,
    async (req, res) => {
      const data = req.body.data;
      console.log(data);
      ...
      try {
        ...
        const msg =
          'The user was successfully deleted and its dependencies merged.';
        res.status(200).send({
          html: `<p>${msg}</p>
               <p><a href="../../../">Click here to continue</a>.</p>`,
        });
