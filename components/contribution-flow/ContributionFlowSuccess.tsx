...
import type {
  NewContributionFlowOrderSuccessQuery,
  NewContributionFlowOrderSuccessQueryVariables,
...
const ContributionFlowSuccess = ({ collective, isEmbed }) => {
  ...
  const stripeErrorMessage = useMemo(() => {
    if (!paymentIntentResult) {
      return null;
    } else if (paymentIntentResult.error) {
      return paymentIntentResult.error.message;
    } else if (!['succeeded', 'processing'].includes(paymentIntentResult.paymentIntent.status)) {
      return (
  ...
  }, [order, loaded, stripeErrorMessage, router, collective]);
