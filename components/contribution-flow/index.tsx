...
import type {
  ...
  Currency,
  OrderResponseFragmentFragment,
  OrderSuccessFragmentFragment,
...
import ContributionFlowSuccessUntyped from './ContributionFlowSuccess';
...
type ContributionFlowCollective = ContributionFlowAccountFieldsFragment & {
  platformContributionAvailable?: boolean;
  host?: ContributionFlowHostFieldsFragment;
  parent?: {
    slug?: string;
    id?: string;
    name?: string;
    ...
    location?: { id?: string; country?: string };
  };
  policies?: {
    id: string;
...
type ContributionFlowQueryParams = {
  amount?: number;
  quantity?: number;
  interval?: string;
  ...
  legalName?: string;
  redirect?: string;
  shouldRedirectParent?: string | boolean;
  tags?: string[];
  hideHeader?: boolean;
  hideSteps?: boolean;
  hideFAQ?: boolean;
...
type ContributionFlowTier = {
  ...
  singleTicket?: boolean | null;
  requireAddress?: boolean | null;
  maxQuantity?: number | null;
  endsAt?: string | null;
  amount: { valueInCents?: number | null; currency?: Currency | null };
  amountType?: string | null;
  minimumAmount?: { valueInCents?: number | null; currency?: Currency | null };
...
type LoggedInStepProfile = {
  ...
  captcha?: string;
  categories?: string[];
  contributorRejectedCategories?: string[];
  host?: { id?: string };
};

type StepProfile = GuestStepProfile | LoggedInStepProfile | null;
...
type StepDetails = {
  ...
  amount: number;
  platformTip: number;
  platformTipOption: PlatformTipOption;
  isNewPlatformTip: boolean;
...
type StepPayment = {
  ...
  stripeData?: {
    stripe?: Stripe & { stripeAccount?: string };
...
type ContributionFlowState = {
  ...
  showSignIn: boolean;
  createdOrder: OrderSuccessFragmentFragment | null;
  forceSummaryStep: boolean;
...
type ContributionFlowProps = {
  collective: ContributionFlowCollective;
  contributorProfiles?: ContributorProfile[];
  host: ContributionFlowHostFieldsFragment;
  tier?: ContributionFlowTier | null;
  isEmbed?: boolean;
  error?: string;
  onStepChange?: (stepName: string) => void;
  onSuccess?: (order: OrderSuccessFragmentFragment) => void;
};
...
type GetInitialStateParams = {
  ...
  queryParams: ContributionFlowQueryParams;
  getCurrentStepName: () => string;
  getDefaultStepProfile: () => StepProfile;
  canHavePlatformTips: () => boolean;
};

type PushStepRouteOptions = {
...
const ContributionFlowSuccess = ContributionFlowSuccessUntyped as React.ComponentType<{
  collective: ContributionFlowCollective;
}>;
...
function getInitialState({
  ...
  canHavePlatformTips,
}: GetInitialStateParams): ContributionFlowState {
  const currency = tier?.amount?.currency || collective.currency;
  const amount = (queryParams.amount as number) || getDefaultTierAmount(tier, collective, currency);
  const quantity = (queryParams.quantity as number) || 1;
  ...
  return {
    ...
    stepPayment: {
      key: queryParams.paymentMethod as string,
      isKeyOnly: true, // For the step payment to recognize if it needs to load the payment method
    },
    stepSummary: null,
    stepDetails: {
      quantity,
      interval: isSupportedInterval(collective, tier, LoggedInUser, queryParams.interval as string)
        ? (queryParams.interval as string)
...
const ContributionFlow = ({
  ...
  const getQueryParams = useCallback((): ContributionFlowQueryParams => {
    return getQueryParamsMemoized(router.query) as ContributionFlowQueryParams;
  }, [getQueryParamsMemoized, router.query]);

  const getCurrentStepName = useCallback((): string => {
    return getQueryStringParam(router.query.step as string | string[]) || STEPS.DETAILS;
  }, [router.query.step]);
  ...
  }, [collective.platformContributionAvailable, tier]);

  const getDefaultStepProfile = useCallback((): StepProfile => {
    const profiles = contributorProfiles || [];
    const queryParams = getQueryParams();

    // If there's a default profile set in contributeAs, use it
    let contributorProfile: ContributorProfile | undefined;
    ...
    if (contributorProfile) {
      return contributorProfile.account as LoggedInStepProfile;
    } else if (profiles[0]?.account) {
      // Otherwise to the logged-in user personal profile, if any
      return profiles[0].account as LoggedInStepProfile;
    }
  ...
  const [state, setState] = useState<ContributionFlowState>(() => {
    const queryParams = (isEmbed ? EmbedContributionFlowUrlQueryHelper : ContributionFlowUrlQueryHelper).decode(
      router.query,
    ) as ContributionFlowQueryParams;
    const getCurrentStepNameForInit = (): string =>
      getQueryStringParam(router.query.step as string | string[]) || STEPS.DETAILS;
  ...
  const stateRef = useRef<ContributionFlowState>(state);
  ...
  const pushStepRoute = useCallback(
    ...
    async (stepName: string, { query: newQueryParams, replace = false }: PushStepRouteOptions = {}) => {
      // Reset errors if any
  ...
  const updateRouteFromState = useCallback(
    async (stateOverride: ContributionFlowState | null = null) => {
  ...
  const handleSuccess = useCallback(
    async (order: OrderSuccessFragmentFragment) => {
  ...
  const handleOrderResponse = useCallback(
    async ({ order, stripeError, guestToken }: OrderResponseFragmentFragment, email?: string) => {
      ...
      if (
        ...
        try {
          await confirmPayment(stripeData?.stripe, stripeData?.paymentIntentClientSecret, {
            ...
            type: stepPayment.paymentMethod?.type as never,
            paymentMethodId: (stepPayment.paymentMethod?.data as { stripePaymentMethodId?: string })
  ...
  const handleStripeError = useCallback(
    async (order: OrderSuccessFragmentFragment, stripeError: StripeError, email: string, guestToken?: string) => {
  ...
  const getPaymentMethod = useCallback(async () => {
    ...
    const paymentMethod: Record<string, unknown> = pick(stepPayment.paymentMethod, [
      'service',
      'type',
    ...
    if (stepPayment.paymentMethod.id) {
      ...
      const pm = stripeTokenToPaymentMethod(token as unknown as { id: string; card: Record<string, unknown> });
      ...
      paymentMethod.paypalInfo = pick(stepPayment.paymentMethod.paypalInfo as Record<string, unknown>, paypalFields);
      // Define the right type (doesn't matter that much today, but make it future proof)
      if ((paymentMethod.paypalInfo as { subscriptionId?: string }).subscriptionId) {
  ...
  const submitOrderRef = useRef<(() => Promise<void>) | null>(null);
  ...
  const submitOrder = useCallback(async () => {
    ...
    let fromAccount: { id: string } | { legacyId: number } | undefined;
    let guestInfo: ReturnType<typeof getGuestInfoFromStepProfile> | undefined;
    ...
    try {
      ...
      const skipTaxes =
        !totalAmount || isEmpty(memoizedGetApplicableTaxes(collective as never, host as never, tier?.type as never));
      const response = await createOrder({
        variables: {
          order: {
            quantity: stepDetails.quantity,
            amount: { valueInCents: stepDetails.amount },
            frequency: getGQLV2FrequencyFromInterval(stepDetails.interval as never),
            guestInfo,
            fromAccount,
            fromAccountInfo: {
  ...
  const validateStepProfile = useCallback(
    ...
    async (action?: string) => {
      ...
      const containsRejectedCategories = getContributorRejectedCategories(stepProfile as LoggedInStepProfile);
  ...
  const getSteps = useCallback(
    ...
    () => {
      ...
      const steps: Array<{
        ...
        label: string;
        isCompleted: boolean;
        validate?: (action?: string) => boolean | Promise<boolean>;
      }> = [
        {
          name: 'details',
        ...
        steps.push({
          ...
          isCompleted:
            !(
              stepProfile &&
              !stepProfile.isGuest &&
              (stepProfile as LoggedInStepProfile).contributorRejectedCategories
          ...
          validate: action => {
            if (action === 'prev') {
              ...
              if (
                ...
                return (stepPayment.paymentMethod?.stripeData as { complete?: boolean })?.complete;
              } else {
                return isCompleted;
              }
            }
    ...
    ({ currency }: { currency: Currency | string }) => {
      ...
      if (stepPayment?.paymentMethod?.service === PAYMENT_METHOD_SERVICE.PAYPAL) {
        return {
          ...
          onError: (e: Error) => setState(prev => ({ ...prev, isSubmitting: false, error: e.message })),
          // New callback, used by `PayWithPaypalButton`
          onSuccess: (paypalInfo: Record<string, unknown>) => {
            setState(prev => {
              const next = {
                ...prev,
                stepPayment: {
  ...
  const setStateAndUpdateRoute = useCallback(
    (data: Partial<ContributionFlowState>) => {
            ...
            >
              ...
              <Box as="form" ref={formRef} onSubmit={e => e.preventDefault()} maxWidth="100%">
                ...
                <ContributionFlowStepContainer
                  ...
                  taxes={memoizedGetApplicableTaxes(collective as never, host as never, tier?.type as never)}
