...
const Footer = ({ simplifiedFooter = true }) => {
  const intl = useIntl();
    ...
    return (
      <footer className="mx-auto flex w-full justify-center border-t p-4">
        <div className="flex w-full max-w-screen-xl flex-col">
          ...
          <div className="my-3 flex w-full justify-between text-sm text-slate-500">
            {[
              ...
              { label: 'Contact us', href: '/tos' },
            ].map(item => (
              <Link key={item.label} href={item.href} className="hover:text-slate-800 hover:underline">
                {item.label}
              </Link>
            ))}
