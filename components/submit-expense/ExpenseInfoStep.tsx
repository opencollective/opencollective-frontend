    ...
    <div>
      ...
      <div className="mt-4 flex flex-col gap-4">
        <FormikProvider value={props.form}>
          ...
          {props.form.values.expenseTypeOption === expenseTypes.INVOICE && (
            <StyledInputFormikField
              name="reference"
              labelFontWeight="bold"
              labelColor="slate.800"
            ...
            >
              {({ field }) => (
                <StyledInput
                  {...field}
                  placeholder={intl.formatMessage(I18nMessages.referencePlaceholder)}
                  className="w-full"
                  onChange={e => props.form.setFieldValue(field.name, e.target.value)}
                />
              )}
            </StyledInputFormikField>
          )}
