...
import { Account, ConnectedAccount } from '@/lib/graphql/types/v2/schema';
...
import { EditTransferWiseAccountQuery } from '@/lib/graphql/types/v2/graphql';
...
const EditTransferWiseAccount = ({ collective }: { collective: Account }) => {
  ...
  const handleDisconnectThunk =
    (connectedAccount: EditTransferWiseAccountQuery['account']['connectedAccounts'][number]) => async () => {
      const action = async () => {
        ...
        <Table className="w-full">
          <TableBody>
            {connectedAccounts.map(connectedAccount => (
              <TableRow key={connectedAccount.id} className="text-sm text-gray-700">
                <TableCell className="w-fit min-w-0">
                  <Badge size="sm">{connectedAccount.hash}</Badge>
                </TableCell>
                  ...
                  {connectedAccount.settings.mirroredCollective && (
                    <p className="text-xs text-gray-500">
                      <FormattedMessage
                        ...
                        values={{
                          name: connectedAccount.settings.mirroredCollective.name,
                        }}
                      />
                    </p>
                  )}
                ...
                <TableCell className="w-fit min-w-0">
                  <Button
                    ...
                    className="w-fit"
                    variant="outline"
                    onClick={handleDisconnectThunk(connectedAccount)}
                    disabled={deleting}
                  >
