...
const leaveHostQuery = gql`
  query AccountInfoForLeaveHost($slug: String) {
    account(slug: $slug, throwIfMissing: true) {
      ...
      name
      stats {
        nonPortableContributions: activeRecurringContributionsBreakdown(hasPortability: false) {
          label
          count
          amount {
            valueInCents
...
const getPortabilitySummary = (
  ...
  const stats = get(account, 'stats.nonPortableContributions');
  if (!stats || !stats.length) {
    return { count: 0, yearlyAmount: 0 };
  }

  const totalCount = stats.reduce((acc, { count }) => acc + count, 0);
  const yearlyAmount = stats.reduce((acc, { label, amount }) => {
    if (label === 'yearly') {
  ...
  return { count: totalCount, yearlyAmount };
};
  ...
  return (
    <StyledModal width="570px" onClose={onClose}>
      ...
      {loading ? (
        ...
        <FormikZod
          schema={portabilitySummary.count ? LeaveHostFormSchemaWithRecurringContributions : LeaveHostFormSchema}
          initialValues={{
            ...
            <Form>
              <ModalBody>
                <div className="mt-3 ">
                  ...
                  {Boolean(portabilitySummary.count) && (
                    <div>
                      <p className="mb-3 text-sm">
                        <FormattedMessage
                          defaultMessage="{accountName} currently has {count, plural, one {# recurring contribution} other {# recurring contributions}} ({yearlyAmount}) that cannot be automatically transferred to another Fiscal Host."
                          values={{
                            accountName: <strong>{account.name}</strong>,
                            count: portabilitySummary.count,
                            yearlyAmount: (
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <label className="text-sm font-normal" htmlFor="pause">
                            ...
                            <p className="pt-1 text-sm text-gray-700">
                              <FormattedMessage defaultMessage="Select this option if you intend to stay on Open Collective (either with another fiscal host or as an independent collective). We will pause all recurring contributions that cannot be automatically transferred and notify your contributors that they can renew them as soon as you are ready." />
                            </p>
                          </label>
                        </div>
                        <div className="flex items-center space-x-2">
