...
export const EditAccountInfoForm = ({ accountSlug }) => {
  ...
  const { data, loading, error, client } = useQuery(ACCOUNT_INFO_QUERY, {
    ...
    <FormikZod
      ...
      onSubmit={async values => {
        try {
          ...
          // Refetching the logged in user to update the user menu
          if (LoggedInUser.collective.id === account.legacyId) {
            refetchLoggedInUser();
          }

          // Update GraphQL cache. Should be inverted to update GraphQL V1 cache once we migrate the mutation to V2.
          client.cache.modify({
            id: client.cache.identify(data.account),
            fields: {
              name: () => values.name,
              slug: () => values.slug,
          ...
          toast({
            variant: 'success',
            title: intl.formatMessage({ defaultMessage: 'Changes saved', id: 'nQKiSH' }),
          });
        } catch (e) {
          toast({
            variant: 'error',
          ...
          <WarnIfUnsavedChanges hasUnsavedChanges={dirty}>
            <Form>
              ...
              <div className="mt-8 flex flex-col items-center">
                <Button type="submit" disabled={!dirty} className="w-full" loading={isSubmitting}>
