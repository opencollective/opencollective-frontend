...
const toggleFeatureMutation = gql`
  mutation ToggleFeature($account: AccountReferenceInput!, $feature: FeatureToggleInput!) {
    toggleFeature(account: $account, feature: $feature) {
      id
      name
      features {
        HOST_ACCOUNTS
...
const FiscalHosting = ({ collective }) => {
  ...
  const canHostAccounts = isFeatureEnabled(collective, FEATURES.HOST_ACCOUNTS);
  ...
  const [activateAsFiscalEntity] = useMutation(activateCollectiveAsHostMutation, adminPanelMutationParams);
  const [deactivateCollectiveAsHost] = useMutation(deactivateCollectiveAsHostMutation, adminPanelMutationParams);
  const [toggleFeature] = useMutation(toggleFeatureMutation, {
  ...
  const handleFundManagementUpdate = async ({ id, activate }) => {
    showConfirmationModal({
      ...
      onConfirm: () => {
        if (activate) {
          return activateAsFiscalEntity({ variables: { id } });
        } else {
          return deactivateCollectiveAsHost({ variables: { id } });
        }
      },
