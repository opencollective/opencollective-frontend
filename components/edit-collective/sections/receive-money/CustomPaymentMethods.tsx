      ...
      try {
        await reorderProviders({
          variables: {
            ...
            providers: [...otherProviders, ...updatedProviders].map(p => pick(p, 'id')),
          },
        });
        onRefetch();
      } catch (error: unknown) {
  ...
  return (
    <div className="flex flex-col">
      <CustomPaymentMethodsList
        customPaymentProviders={customProviders.filter(p => !p.isArchived)}
      ...
      {editingId && (
        <EditCustomPaymentMethodDialog
          provider={editingId !== 'new' ? customProviders.find(p => p.id === editingId) : undefined}
          onSave={handleSave}
