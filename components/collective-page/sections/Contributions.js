...
//     const { memberOf, connectedCollectives } = data.Collective;
//     if (!memberOf.length && !connectedCollectives.length) {
//       return null;
//     }
...
const newContributionsSectionQuery = gqlV2/* GraphQL */ `
  query ContributionsSection($slug: String!) {
    account(slug: $slug) {
      ...
      connectedAccounts: members(role: CONNECTED_COLLECTIVE) {
        id
        collective: member {
          id
          name
...
const SectionContributions = ({ collective }) => {
  const { data, loading } = useQuery(newContributionsSectionQuery, {
    variables: getContributionsSectionQueryVariables(collective.slug),
