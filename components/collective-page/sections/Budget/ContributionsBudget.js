...
const budgetSectionContributionsQuery = gqlV2/* GraphQL */ `
  query BudgetSectionContributionsQuery($slug: String!, $from: DateTime, $to: DateTime) {
    account(slug: $slug) {
      ...
      stats {
        id
        contributionsAmount(dateFrom: $from, dateTo: $to, includeChildren: true) {
          label
          count
          amount {
            value
        ...
        contributionsAmountTimeSeries(dateFrom: $from, dateTo: $to, includeChildren: true) {
          timeUnit
          nodes {
            date
            amount {
