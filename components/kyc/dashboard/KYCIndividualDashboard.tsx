...
import type { KycIndividualDashboardQuery } from '@/lib/graphql/types/v2/graphql';
...
export function KYCIndividualDashboard(props: DashboardSectionProps) {
  ...
  const {
    ...
    error: queryError,
  } = useQuery<KycIndividualDashboardQuery>(
    gql`
      query KYCIndividualDashboard($slug: String!) {
        account(slug: $slug) {
          ... on Individual {
            kycVerifications {
  ...
  const kycVerifications =
    data?.account && 'kycVerifications' in data.account
      ? data.account.kycVerifications
