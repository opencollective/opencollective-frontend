...
const ActivityDescription = ({ activity }) => {
  ...
  return intl.formatMessage(ActivityDescriptionI18n[activity.type], {
    ...
    FromAccount: () => (
      <LinkCollective className="text-slate-900 hover:underline" collective={activity.fromAccount} openInNewTab />
    ),
    Account: () => (
      <LinkCollective className="text-slate-900 hover:underline" collective={activity.account} openInNewTab />
    ),
    AccountType: () => formatCollectiveType(intl, activity.account?.type || 'COLLECTIVE'),
    AccountParent: () => (
      <LinkCollective className="text-slate-900 hover:underline" collective={activity.account?.parent} openInNewTab />
    ),
    Expense: msg =>
      !activity.expense ? (
        msg
