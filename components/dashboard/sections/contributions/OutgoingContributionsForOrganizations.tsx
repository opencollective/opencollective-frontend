...
const hostFinancialContributionsMetadataQuery = gql`
  query HostFinancialContributionsMetadata($slug: String!, $hostContext: HostContext) {
    account(slug: $slug) {
      ...
      DISPUTED: orders(filter: OUTGOING, status: [DISPUTED], includeIncognito: true, hostContext: $hostContext) {
        totalCount
      }
      IN_REVIEW: orders(filter: OUTGOING, status: [IN_REVIEW], includeIncognito: true, hostContext: $hostContext) {
        totalCount
      }
      ERROR: orders(filter: OUTGOING, status: [ERROR], includeIncognito: true, hostContext: $hostContext) {
        totalCount
      }
      PAUSED: orders(filter: OUTGOING, status: [PAUSED], includeIncognito: true, hostContext: $hostContext) {
        totalCount
      }
    }
  }
...
export default function OutgoingContributionsForOrganizations({ accountSlug }: DashboardSectionProps) {
  const intl = useIntl();
    ...
    <div className="flex flex-col gap-4">
      <DashboardHeader
        ...
        description={
          hasHosting ? (
            <FormattedMessage
              defaultMessage="Contributions made from your Organization and Collectives you host."
              id="vcsTj4"
            />
          ) : (
            <FormattedMessage defaultMessage="Contributions made from your Organization." id="ArTL6H" />
          )
        }
      />
