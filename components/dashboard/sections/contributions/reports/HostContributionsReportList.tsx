...
import type {
  HostContributionsReportListQuery,
  HostContributionsReportListQueryVariables,
...
const getColumns = intl => [
  ...
  {
    accessorKey: 'managedContributionsTotal',
    meta: { className: 'w-44 text-right' },
    header: intl.formatMessage({ defaultMessage: 'Managed contributions', id: 'QpkT+/' }),
    cell: ({ cell, row }) => {
      const amount = cell.getValue();
      const count = row.original.managedContributionsCount;
      return (
  ...
  {
    accessorKey: 'operationalContributionsTotal',
    meta: { className: 'w-44 text-right' },
    header: intl.formatMessage({ defaultMessage: 'Operational contributions', id: 'lDrehw' }),
    cell: ({ cell, row }) => {
      const amount = cell.getValue();
      const count = row.original.operationalContributionsCount;
      return (
        <div className="inline-flex items-center gap-1 font-normal text-muted-foreground">
          <FormattedMoneyAmount amount={amount.valueInCents} currency={amount.currency} showCurrencyCode={false} />
          <Tooltip delayDuration={100}>
...
export function HostContributionsReportList(props: DashboardSectionProps) {
  const queryFilter = useQueryFilter({
    filters: {},
    schema,
  });

  const query = useQuery<HostContributionsReportListQuery, HostContributionsReportListQueryVariables>(
    gql`
      query HostContributionsReportList(
        $accountSlug: String!
        $timeUnit: TimeUnit
        $dateFrom: DateTime
        $dateTo: DateTime
  ...
  const listData = React.useMemo(() => {
    ...
    const timeUnit = query.data?.host?.hostContributionsReport?.timeUnit;

    const result = [];
    const byDate = groupBy(query.data?.host?.hostContributionsReport?.nodes, 'date');
    Object.entries(byDate).forEach(([date, values]) => {
      const managedValueInCents = sumBy(
        values.filter(v => !v.isHost),
        'amount.valueInCents',
      );
      const managedContributionsCount = sumBy(
        values.filter(v => !v.isHost),
        'count',
      );
      ...
      const operationalContributionsCount = sumBy(
        values.filter(v => v.isHost),
        'count',
      );
      result.push({
        ...
        managedContributionsCount,
        operationalContributionsCount,
        managedContributionsTotal: {
          currency: values[0].amount.currency,
          valueInCents: managedValueInCents,
        },
        operationalContributionsTotal: {
          currency: values[0].amount.currency,
  ...
  }, [query.data?.host?.hostContributionsReport?.nodes, query.data?.host?.hostContributionsReport?.timeUnit]);
  ...
  return (
    <React.Fragment>
      <div className="flex flex-col gap-4">
        <DashboardHeader
          title={<FormattedMessage defaultMessage="Contribution Reports" id="GjWVzo" />}
                ...
                <DataTable
                  ...
                  onClickRow={row => {
                    queryFilter.resetFilters(
                      {},
                      `/dashboard/${props.accountSlug}/reports/contributions/${serializeReportSlug(row.original.period)}`,
                    );
                  }}
                />
              </div>
              <Pagination queryFilter={queryFilter} total={query.data?.host?.hostContributionsReport?.nodes?.length} />
            </React.Fragment>
          )}
        </div>
      </div>
