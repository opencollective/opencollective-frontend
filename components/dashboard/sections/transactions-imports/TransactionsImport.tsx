...
export const TransactionsImport = ({ accountSlug, importId }) => {
  ...
  const [hasNewData, setHasNewData] = React.useState(true);
  ...
  useQuery(transactionsImportLasSyncAtPollQuery, {
    ...
    variables: { importId },
    pollInterval: !importData?.lastSyncAt ? 1_000 : 10_000, // Poll every second if the first sync is in progress, otherwise every 10 seconds
        ...
        <React.Fragment>
          ...
          {!hasStepper && (
            <div>
              ...
              {hasNewData && (
                <div className="sticky top-0 z-50 mb-6 flex items-center justify-between rounded-none rounded-bl-lg rounded-br-lg bg-blue-500 px-4 py-2 text-sm text-white">
