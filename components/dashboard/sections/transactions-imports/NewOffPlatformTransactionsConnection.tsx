...
const InstitutionStep = ({
  ...
  const filteredInstitutions = React.useMemo(() => {
    if (!institutionsData?.offPlatformTransactionsInstitutions || !institutionFilter.trim()) {
      return institutionsData?.offPlatformTransactionsInstitutions || [];
    }

    const filter = institutionFilter.toLowerCase().trim();
    return institutionsData.offPlatformTransactionsInstitutions.filter(
      institution =>
