...
class CreateUpdate extends React.Component {
  ...
  constructor(props) {
    ...
    this.messages = defineMessages({
      ...
      'newUpdate.paypal.label': { id: 'newUpdate.paypal.label', defaultMessage: 'Please provide address' },
  ...
  handleChange(attr, value) {
    ...
    const newState = { modified: true, expense, isUpdateValid: this.validate(expense) };
  ...
  async onSubmit(e) {
    ...
    try {
      ...
      this.setState({ modified: false, isUpdateValid: false, expense: {}, loading: false });
    } catch (e) {
      this.setState({ loading: false });
      console.error("CreateUpdateForm onSubmit error", e);
    }
    return false;
  }
  ...
  render() {
    ...
    const payoutMethods = this.getOptions(['paypal', 'other'], { paypalEmail: get(expense, 'user.paypalEmail') || intl.formatMessage(this.messages['newUpdate.paypal.label']) });

    return (
        <div className={`CreateUpdate ${this.props.mode}`}>
        <style jsx>{`
          .CreateUpdate {
            font-size: 1.2rem;
            overflow: hidden;
        ...
        `}</style>
        <style global jsx>{`
          .CreateUpdate .inputField {
            margin: 0;
          }

          .CreateUpdate .descriptionField {
            width: 50rem;
            max-width: 100%;
          }

          .CreateUpdate .amountField {
            max-width: 15rem;
          }

          .CreateUpdate .inputField textarea {
            font-size: 1.2rem;
          }

          .CreateUpdate .attachmentField {
            width: 128px;
          }

          .CreateUpdate .attachmentField .form-group {
            margin: 0;
          }

          .CreateUpdate .col.privateMessage {
            width: 100%;
          }

          .CreateUpdate .help-block {
            font-size: 1.2rem;
          }
        ...
        <form onSubmit={this.onSubmit}>
          ...
          <div className="rightColumn">
            ...
            <div className="row">
              <div className="col large">
                <Button bsStyle="primary" type="submit" ref="submit" disabled= {this.state.loading || !this.state.isUpdateValid} >
                  { this.state.loading && <FormattedMessage id="form.processing" defaultMessage="processing" /> }
                  { !this.state.loading && <FormattedMessage id="expense.new.submit" defaultMessage="Submit Update" /> }
                </Button>
              </div>
            </div>
...
export default withIntl(CreateUpdate);