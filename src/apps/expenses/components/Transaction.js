      ...
      <Flex my={4}>
        ...
        <Container ml={3} width={1}>
          <Flex justifyContent="space-between" alignItems="center">
            <div>
              <P fontSize="1.4rem" color="#9399A3" display="inline">
                {description}
                {type === 'DEBIT' && ' expense '}
                { collective && <Fragment>{' to '} <Link route={`/${collective.slug}`} title={collective.name}>{collective.name}</Link>.</Fragment> }
              </P>
            ...
            <Flex alignItems="baseline">
              ...
              <Box ml={2}>
                <object type="image/svg+xml" data={`/static/icons/${type.toLowerCase()}-arrow.svg`} height="16"></object>
              </Box>
            </Flex>
          </Flex>
          <Container fontSize="1.2rem" color="#AEB2B8">
            ...
            { ' | ' }
            <span title={moment(formattedCreatedAt).format('LLLL')}>{moment(formattedCreatedAt).fromNow()}</span>
