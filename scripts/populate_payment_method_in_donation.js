...
function run() {
  ...
  return models.Order.findAll({ where: { PaymentMethodId: null } })
    .each(order => {
      ...
      return models.Transaction.findAll({ where: { OrderId: order.id } }).then(
        transactions => {
          if (
            transactions &&
            transactions.length >= 1 &&
            transactions[0].PaymentMethodId != null
          ) {
            updatedCount += 1;
            console.log(
              `Setting OrderId: ${order.id} with PaymentMethodId: ${
                transactions[0].PaymentMethodId
              }`,
            );
