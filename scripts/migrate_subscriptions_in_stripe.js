...
const migrateSubscriptions = options => {
  ...
  return (
    models.ConnectedAccount.findAll({
      where: { CollectiveId: oldHostCollectiveId },
    })
      .then(stripeAccounts => {
      ...
      .then(() =>
        models.ConnectedAccount.findAll({
          where: { CollectiveId: newHostCollectiveId },
        }),
      )
      .then(stripeAccounts => {
        if (!stripeAccounts || stripeAccounts.length === 0) {
      ...
      .then(() =>
        stripeGateway.getSubscriptionsList(oldStripeAccount, {
          limit,
          startingAfter,
          endingBefore,
          plan,
      ...
      .each(oldStripeSubscription => {
        ...
        return (
          models.Subscription.findOne({
            where: { stripeSubscriptionId: oldStripeSubscription.id },
          })
            .then(ocSubscription => {
            ...
            .then(order => {
              ...
              const processSubscription = () => {
                ...
                return (
                  stripeGateway
                    .getOrCreatePlan(newStripeAccount, plan)
        ...
        */
              ...
              if (
                ...
                return stripeGateway
                  .createToken(newStripeAccount, platformCustomerId)
                  .then(token =>
                    stripeGateway.createCustomer(newStripeAccount, token.id, {
...
const run = () => {
  ...
  parser.addArgument(['-f', '--fromHostCollectiveId'], {
    help: 'Stripe ConnectedAccount Id of Host to move FROM',
    required: true,
  });
  parser.addArgument(['-t', '--toHostCollectiveId'], {
  ...
  parser.addArgument(['--starting_after'], {
    help: 'subscription id on Stripe to start after',
    defaultValue: null,
  });
  parser.addArgument(['--ending_before'], {
