...
export function addAuthTokenToHeader(obj = {}): Record<string, string> {
...
export const mockImageUpload = (
  mockImageGenerator: string | (() => string) = 'https://d.pr/free/i/OlQVIb+',
  {
    onProgress,
    timeToWait = 1000,
    nbStepsProgress = 10,
...
export function connectAccount(CollectiveId, service, options: { redirect?: string } = {}) {
  const params = {
...
export function connectAccountCallback(CollectiveId, service, options: { redirect?: string } = {}) {
  const params = {
    redirect: options.redirect || window.location.href.replace(/\?.*/, ''),
    CollectiveId,
    ...options,
 ...
 */
export function getPaypalConnectConfig(collectiveId: number): Promise<{
  clientId: string;
  redirectUri: string;
  authorizeUrl: string;
} | null> {
  const url = new URL(`${window.location.origin}/api/connected-accounts/paypal/connect-config`);
  url.searchParams.set('CollectiveId', collectiveId.toString());
 ...
 */
export function get(path, options: { format?: string; allowExternal?: string } = {}) {
...
export async function downloadLegalDocument(legalDocument, account, prompt2fa) {
  ...
  const handleResponse = async (response, retryWith2FA) => {
    ...
    if (retryWith2FA && response.status === 401 && responseBody.message === 'Two-factor authentication required') {
