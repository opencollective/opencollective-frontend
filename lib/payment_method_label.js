 ...
 */
export const getPaymentMethodName = ({ name, data, service, type, stripeData }) => {
  if (type === 'giftcard') {
  ...
  } else if (type === 'PAYPAL' || service === GQLV2_PAYMENT_METHOD_TYPES.PAYPAL) {
    return 'PayPal';
  } else if (type === 'CREDIT_CARD' && stripeData) {
    const brand = stripeData.brand && formatCreditCardBrand(stripeData.brand);
    return `${brand || 'Credit card'} **** ${name || ''}`;
