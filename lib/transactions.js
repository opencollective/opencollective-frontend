...
export const getInvoiceUrl = ({ fromCollectiveSlug, toCollectiveSlug, transactionId, dateFrom, dateTo }) => {
  return transactionId
    ? transactionInvoiceURL(transactionId)
    : collectiveInvoiceURL(fromCollectiveSlug, toCollectiveSlug, encodeURI(dateFrom), encodeURI(dateTo), 'pdf');
};

export const getFilename = ({ fromCollectiveSlug, transactionId, toCollectiveSlug, dateFrom, dateTo, createdAt }) => {
  if (transactionId) {
    const createdAtString = toIsoDateStr(createdAt ? new Date(createdAt) : new Date());
    return `${toCollectiveSlug || 'transaction'}_${createdAtString}_${transactionId}.pdf`;
  } else {
...
export const saveInvoice = async ({
  ...
  const filename = getFilename({ fromCollectiveSlug, toCollectiveSlug, transactionId, dateFrom, dateTo, createdAt });
