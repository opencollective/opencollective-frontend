...
import { collectiveInvoiceURL, expenseInvoiceUrl, PDF_SERVICE_V2_URL, transactionInvoiceURL } from './url-helpers';
  ...
  if (kind === TransactionKind.EXPENSE) {
    ...
    const netValueInCents = splitPaymentProcessorFee
      ? Math.abs(_netAmount.valueInCents) -
        (isCredit && feesPayer === 'PAYEE' ? Math.abs(paymentProcessorFee.valueInCents) : 0)
      : Math.abs(_netAmount.valueInCents);
...
export const saveInvoice = async ({
  ...
  const getParams = { format: 'blob', allowExternal: PDF_SERVICE_V2_URL };
