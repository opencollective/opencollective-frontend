...
export const renderDetailsString = ({
  ...
  if (kind === TransactionKind.EXPENSE) {
    ...
    const netExpenseAmount = formatCurrency(Math.abs(_netAmount.valueInCents), amount.currency, {
      locale: intl.locale,
    });
    const paymentProcessorCoverExits = paymentProcessorCover !== undefined;
    const netPaymentProcessorCoverAmount = paymentProcessorCoverExits
      ? formatCurrency(
    ...
    const feesPayer = expense.feesPayer;

    return (
      <React.Fragment>
        <Container mb={1}>
          {paymentProcessorFee.valueInCents !== 0 || paymentProcessorCoverExits ? (
          ...
          <React.Fragment>
            <Container mb={1}>
              {!isRefund ? (
                ...
                <FormattedMessage
                  defaultMessage="Refunded Payment Processor Fee{hasPaymentProcessorCover, select, true { (covered by the Host)} false {} other {}}: {refundedPaymentProcessorFee}"
                  values={{
                    refundedPaymentProcessorFee: paymentProcessorCoverExits
                      ? netPaymentProcessorCoverAmount
                      : expensePaymentProcessorFee,
                    hasPaymentProcessorCover: paymentProcessorCoverExits,
                  }}
                />
              )}
            </Container>
