...
export const renderDetailsString = ({
  ...
  if (kind === TransactionKind.EXPENSE) {
    const expenseAmount = formatCurrency(Math.abs(_amount.valueInCents), _amount.currency, { locale: intl.locale });
    ...
    return (
      <React.Fragment>
        <Container mb={1}>
          {isCredit ? (
            <FormattedMessage defaultMessage="Amount from {payer}: {expenseAmount}" values={{ payer, expenseAmount }} />
          ) : (
        ...
        <Container mb={1}>
          <FormattedMessage
            defaultMessage="Payment Processor Fee: {expensePaymentProcessorFee}"
            values={{ expensePaymentProcessorFee }}
          />
        </Container>
        <Container>
          {isCredit ? (
            <FormattedMessage
              defaultMessage="Net Amount for {payee}: {netExpenseAmount}"
              values={{ payee, netExpenseAmount }}
            />
          ) : (
            <FormattedMessage
              defaultMessage="Net Amount from {payer}: {netExpenseAmount}"
              values={{ payer, netExpenseAmount }}
            />
          )}
        </Container>
      </React.Fragment>
