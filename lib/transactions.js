    ...
    return (
      <React.Fragment>
        <Container mb={1}>
          {isCredit ? (
            ...
            <FormattedMessage
              ...
              values={{ expenseAmount: feesPayer === 'PAYEE' ? netExpenseAmount : expenseAmount }}
            />
          )}
        </Container>
        {paymentProcessorFee.valueInCents !== 0 && (
          <Container mb={1}>
            <FormattedMessage
              defaultMessage="Payment Processor Fee (from {payer}): {expensePaymentProcessorFee}"
              values={{ payer: feesPayer === 'PAYEE' ? payee : payer, expensePaymentProcessorFee }}
            />
          </Container>
        )}
      </React.Fragment>
