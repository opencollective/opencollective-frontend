...
export function isProtectedUploadedFileUrl(url) {
  try {
    const parsedUrl = new URL(url);
    return parsedUrl.origin === getWebsiteUrl() && /^\/api\/files\//.test(parsedUrl.pathname);
  } catch {
    return false;
  }
}

export function isFrontendStaticImage(url) {
  try {
    const parsedUrl = new URL(url);
    return parsedUrl.origin === getWebsiteUrl() && /^\/static\/images\//.test(parsedUrl.pathname);
  } catch {
    return false;
  }
}
 ...
 * @returns {Promise<{width: number, height: number} | null>} - Image dimensions or null if invalid/not an image
 */
