...
import { getApiUrl, getWebsiteUrl } from './utils';
...
export function generateDownloadUrlForFileUrl(fileUrl) {
  try {
    ...
    return `${getApiUrl()}/download-file?url=${encodeURIComponent(fileUrl)}`;
  } catch {
    return fileUrl;
  }
}
...
function resizeImage(imageUrl, { width, height, query, baseUrl }) {
  ...
  try {
    ...
    if (url.origin === getApiUrl() && /^\/(?:api\/)?files\//.test(url.pathname)) {
