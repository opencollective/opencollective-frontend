...
export const useImageUploader = ({ isMulti, mockImageGenerator, onSuccess, onReject = undefined, kind, accept }) => {
    ...
    uploadFiles: React.useCallback(
      async (acceptedFiles, rejectedFiles) => {
        ...
        if (rejectedFiles?.length) {
          toast({
            variant: 'error',
            message: getMessageForRejectedDropzoneFiles(intl, rejectedFiles, accept),
          });
        }
      },
...
export const getMessageForRejectedDropzoneFiles = (intl, rejectedFiles, accept) => {
  ...
  const errorMsg = msg[code] ? intl.formatMessage(msg[code], { ...I18nFormatters, availableExtensions }) : message;
