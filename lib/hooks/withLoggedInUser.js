...
function jwtDecode(token) {
  if (!process.browser) {
    throw new Error('Decoding JWT token from the server side is not supported.');
  }
...
const maybeRefreshAccessToken = async currentToken => {
  const decodeResult = jwtDecode(currentToken);
  if (!decodeResult) {
    return null;
  }
  ...
  return class withLoggedInUser extends React.Component {
    ...
    getLoggedInUser = async (options = {}) => {
      ...
      if (token) {
        ...
        const decodedLocalStorageToken = jwtDecode(localStorageToken);
