...
const maybeRefreshAccessToken = async currentToken => {
  ...
  if (shouldUpdate) {
    // call to API again to exchange for session token or 2FA token
  ...
  return class withLoggedInUser extends React.Component {
    ...
    getLoggedInUser = async (options = {}) => {
      ...
      if (token) {
        ...
        // TODO(oauth-mode): is this helpful to read session token here?
        // const session = await getSession();
        // const localToken = getFromLocalStorage(LOCAL_STORAGE_KEYS.ACCESS_TOKEN) || session?.accessToken;

        const localToken = getFromLocalStorage(LOCAL_STORAGE_KEYS.ACCESS_TOKEN);

        if (!localToken) {
          return null;
        }

        const decodedLocalStorageToken = jwt.decode(localToken);
        if (decodedLocalStorageToken.scope === 'twofactorauth') {
          return null;
        }

        // refresh Access Token in the background if needed
        await maybeRefreshAccessToken(localToken);
      }
