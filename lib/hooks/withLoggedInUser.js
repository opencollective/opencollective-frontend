...
import {
  exchangeLoginToken,
  refreshToken,
  refreshTokenWithTwoFactorCode,
  refreshTokenWithTwoFactorRecoveryCode,
} from '../api';
      ...
      if (token) {
        ...
        // We received directly a 'twofactorauth' prompt after login in with password
          ...
          if (twoFactorAuthenticatorCode) {
            const newToken = await refreshTokenWithTwoFactorCode(token, twoFactorAuthenticatorCode);
            setLocalStorage(LOCAL_STORAGE_KEYS.ACCESS_TOKEN, newToken);
          } else if (recoveryCode) {
            const newToken = await refreshTokenWithTwoFactorRecoveryCode(token, recoveryCode);
            setLocalStorage(LOCAL_STORAGE_KEYS.ACCESS_TOKEN, newToken);
          } else {
            setLocalStorage(LOCAL_STORAGE_KEYS.ACCESS_TOKEN, token);
          ...
          throw new Error(`Unsupported scope: ${decodedNewToken.scope}`);
        }
      } else {
