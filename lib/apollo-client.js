...
const getBaseApiUrl = apiVersion => {
  if (process.browser) {
    return '/api';
  }

  if (apiVersion === 'v1' && process.env.INTERNAL_API_V1_URL) {
    return process.env.INTERNAL_API_V1_URL;
  }

  if (apiVersion === 'v2' && process.env.INTERNAL_API_V2_URL) {
...
const getGraphqlUrl = apiVersion => {
  ...
  return `${getBaseApiUrl(apiVersion)}/graphql/${apiVersion}${apiKey ? `?api_key=${apiKey}` : ''}`;
};
