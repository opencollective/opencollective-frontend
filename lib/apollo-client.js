...
const serverSideFetch = async (url, options = {}) => {
  ...
  if (body && body.variables && body.variables.slug) {
    debugPerformance('serverSideFetch: start', body.variables.slug);
  }
  ...
  if (process.env.GRAPHQL_BENCHMARK) {
    ...
    if (body.operationName || body.variables) {
      ...
      if (variables.slug) {
        debugPerformance('serverSideFetch: end', variables.slug);
      }
    }
  }
