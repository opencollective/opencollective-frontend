...
/**
 * Given number inputs (example: currency values) returns errors for all fields which are not numbers.
 */
export const validateNumberInputs = (data, numberInputs, { stopOnFirstError = false } = {}) => {
  const errors = {};

  for (const field of numberInputs) {
    const value = get(data, field);
    if (isNaN(value)) {
      set(errors, field, createError(ERROR.FORM_FIELD_INVALID_NUMBER));
    }
    if (stopOnFirstError) {
      return errors;
    }
