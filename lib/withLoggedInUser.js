...
import { refreshToken, refreshTokenWithRecoveryCode, refreshTokenWithTwoFactorAuthentication } from './api';
...
const verifyTwoFactorAuthenticationRecoveryCode = async (currentToken, recoveryCode) => {
  if (recoveryCode) {
    const res = await refreshTokenWithRecoveryCode(currentToken, recoveryCode);
    const { token, error } = res;
    if (error) {
      return error;
    } else if (token) {
...
const withLoggedInUser = WrappedComponent => {
  return class withLoggedInUser extends React.Component {
    ...
    getLoggedInUser = async (options = {}) => {
      const { ignoreLocalStorage = false, token = null, twoFactorAuthenticatorCode, recoveryCode } = options;
      ...
      if (token) {
        ...
        if (twoFactorAuthenticatorCode) {
          ...
          newToken = await verifyTwoFactorAuthenticationRecoveryCode(token, recoveryCode);
          if (newToken.code === 401) {
            // if we receive a 401 error from the API this means the 2FA recovery code has failed
            throw new Error(newToken.message);
          }
        } else {
