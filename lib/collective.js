...
export const expenseSubmissionAllowed = (collective, user) => {
  ...
  return user?.memberOf.some(member => member.account.slug === collective.slug);
};
...
export const canContributeRecurring = (collective, user) => {
  ...
  return Boolean(
    user?.memberOf.some(
      member =>
        member.account?.host?.id === hostId && // Must be under the same host
        member.account.id !== collectiveId && // Must not be the same collective
