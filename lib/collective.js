import { get, mapValues, orderBy, partition, trim, truncate } from 'lodash';
...
export const hasAccountMoneyManagement = account => account && account.isHost === true;

/* Returns true if the account has Hosting activated. */
export const hasAccountHosting = account =>
  account && account.isHost === true && account.type !== 'COLLECTIVE' && account.settings?.canHostAccounts !== false;
...
/* Returns true if the account is either a Project or Event, e.g a child account */
export const isChildAccount = account => ['PROJECT', 'EVENT'].includes(account.type);
...
export const isHiddenAccount = account => {
  return !account || account.isSuspended || account.type === 'VENDOR';
};
