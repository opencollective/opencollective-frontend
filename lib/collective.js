import { get, mapValues, orderBy, partition, trim, truncate } from 'lodash-es';
...
/* Returns true if the account has Money Management activated */
export const hasAccountMoneyManagement = account => {
  if (typeof account?.hasMoneyManagement === 'boolean') {
    return account.hasMoneyManagement;
  } else if (typeof account?.isHost === 'boolean') {
...
/* Returns true if the account is an organization.*/
export const isOrganizationAccount = account => ['ORGANIZATION'].includes(account.type);
...
/**
 * A helper to check if two accounts are the same. Works with GQLV1 and GQLV2.
 */
export const isSameAccount = (account1, account2) => {
  if (!account1 || !account2) {
    return false;
  }

  // Check on new IDs
  if (typeof account1.id === 'string' && typeof account2.id === 'string') {
    return account1.id === account2.id;
  }

  // Check on legacy ID
  const legacyId1 = getLegacyIdForCollective(account1);
  const legacyId2 = getLegacyIdForCollective(account2);
  if (legacyId1 && legacyId2) {
    return legacyId1 === legacyId2;
  }

  // Fallback on slug
  return account1.slug === account2.slug;
};

/** Whether the expense account is the host org or a direct child (project/event). */
export const isHostExpenseAccount = (host, account) => {
  return isSameAccount(host, account) || isSameAccount(host, account?.parent || account?.parentCollective);
};
...
export const getCollectivePageMetadata = collective => {
  ...
  if (collective) {
    ...
    return {
      ...
      noRobots: !shouldIndexAccountOnSearchEngines(collective) || hasSomePrivateQueryParams(),
      image:
...
const hasSomePrivateQueryParams = () => {
  if (typeof window === 'undefined') {
    return undefined;
  }

  const privateQueryParams = ['legalName', 'email', 'draftKey'];
  const searchParams = new URLSearchParams(window.location.search);
  return privateQueryParams.some(field => searchParams.has(field));
};

export const isHeavyAccount = accountSlug => {
  return ['opencollective', 'opensource', 'foundation', 'europe', 'ofico', 'ofitech'].includes(accountSlug);
};

/**
 * Returns true if the account is not meant to have a public profile.
 *
 * Prefers the `hasPublicProfile` field (resolved server-side from the `PUBLIC_PROFILE` feature
 * status, which already accounts for `isPrivate`, `isSuspended` and account type) when it was
 * fetched. Falls back to a type based behavior when `hasPublicProfile` is unavailable or not a
 * boolean: vendors default to hidden (no public profile) unless `PUBLIC_PROFILE` is active, while
 * other accounts default to visible unless explicitly private/suspended/disabled/unsupported.
 */
export const isHiddenAccount = account => {
  if (!account) {
    return true;
  } else if (typeof account.hasPublicProfile === 'boolean') {
    return !account.hasPublicProfile;
  }

  // If `hasPublicProfile` was fetched fall-back to VENDOR default behavior.
  if (account.type === CollectiveType.VENDOR) {
    return account.features?.[FEATURES.PUBLIC_PROFILE] !== 'ACTIVE';
  }

  return (
    account.features?.[FEATURES.PUBLIC_PROFILE] === 'UNSUPPORTED' ||
    account.features?.[FEATURES.PUBLIC_PROFILE] === 'DISABLED' ||
    account.isPrivate ||
    account.isSuspended
  );
};

export const sortProjects = (projects, orderKeys) => {
  const [sortedProjects, unsortedProjects] = partition(projects, project => orderKeys.includes(project.id));
  return [
    ...orderBy(sortedProjects, project => orderKeys.findIndex(key => key === project.id)),
    ...orderBy(unsortedProjects, ['deactivatedAt', 'createdAt', 'id'], ['desc', 'desc', 'desc']),
  ];
};

export const sortConnectedCollectives = (connectedCollectives, orderKeys) => {
  const [sortedConnectedCollectives, unsortedConnectedCollectives] = partition(connectedCollectives, collective =>
    orderKeys.includes(collective.id),
  );
  return [
    ...orderBy(sortedConnectedCollectives, collective => orderKeys.findIndex(key => key === collective.id)),
    ...orderBy(unsortedConnectedCollectives, ['deactivatedAt', 'name', 'id'], ['desc', 'asc', 'asc']),
  ];
};

/**
 * Update the collective in the GraphQLV1 cache.
 * @param {ApolloCache} store - The cache to update.
 * @param {number} collectiveId - The ID of the collective to update.
 * @param {Record<string, unknown>} newValues - The new values to update.
 */
export const updateCollectiveInGraphQLV1Cache = async (store, collectiveId, newValues, optimistic = true) => {
  store.modify({
    broadcast: true,
    optimistic: optimistic,
    id: store.identify({ __typename: 'Collective', id: collectiveId }),
    fields: mapValues(newValues, value => () => value), // Apollo expects a getter
  });
};

/**
 *
 * @param {{ settings?: { useAlternativeHostFeeNaming?: boolean } }} account
 * @returns {boolean}
 */
export const checkUseAlternativeHostFeeNaming = account => {
  return Boolean(account?.settings?.useAlternativeHostFeeNaming);
};
