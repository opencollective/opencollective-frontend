...
export const hasAccountHosting = account => {
  if (typeof account?.hasHosting === 'boolean') {
    return account.hasHosting;
  } else if (account) {
    return account.isHost === true && account.type !== 'COLLECTIVE' && account.settings?.canHostAccounts !== false;
  }
};
