...
import { setLocalStorage, getFromLocalStorage, LOCAL_STORAGE_KEYS } from './local-storage';
import { closedBetaAccessAccounts, previewFeatures } from './preview-features';
...
LoggedInUser.prototype.hasPreviewFeatureEnabled = function (featureKey) {
  ...
  return Boolean(earlyAccess[featureKey]);
};
...
LoggedInUser.prototype.getAvailablePreviewFeatures = function () {
   ...
   * - the user is admin/member of an account that have closed beta access by default
   */
  const availablePreviewFeatures = previewFeatures.filter(feature => {
    ...
    return (
      feature.publicBeta ||
      userHaveSetting ||
      closedBetaAccessAccounts.some(slug => this.hasRole([ROLES.ADMIN, ROLES.MEMBER], { slug }))
    );
  });
...
LoggedInUser.prototype.getLastDashboardSlug = function () {
  const slug = getFromLocalStorage(LOCAL_STORAGE_KEYS.LAST_DASHBOARD_SLUG);
  return slug && this.hasRole([ROLES.ADMIN], { slug }) ? slug : this.collective.slug;
};

LoggedInUser.prototype.saveLastDashboardSlug = function (slug) {
  setLocalStorage(LOCAL_STORAGE_KEYS.LAST_DASHBOARD_SLUG, slug);
};
