...
LoggedInUser.prototype.canEditCollective = function(collective) {
  ...
  return collective.id === this.CollectiveId || intersection(this.roles[collective.slug], ['HOST', 'ADMIN']).length > 0;
};
...
LoggedInUser.prototype.canEditComment = function(comment) {
  ...
  return intersection(this.roles[get(comment, 'collective.slug')], ['HOST', 'ADMIN']).length > 0;
};
...
LoggedInUser.prototype.canEditSubscription = function(order) {
  ...
  if (this.roles[order.fromCollective.slug] && this.roles[order.fromCollective.slug].includes('ADMIN')) {
    return true;
  }
  return false;
};
