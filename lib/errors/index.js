 ...
 *
 */
import { get, has, isError } from 'lodash-es';
...
export const getErrorTypeFromHTTPCode = code => {
  switch (code) {
    case 404:
 ...
 * @returns {OCError}
 */
export const generateNotFoundError = (searchTerm = undefined) => {
...
export const getErrorFromGraphqlException = exception => {
  ...
  if (isError(exception) && (exception.networkError || exception.message.startsWith('Network'))) {
    ...
    return createError(errorCode || ERROR.UNKNOWN, { message: exception.message });
  } else {
    return createError();
  }
};
...
const msg = {
  ...defineMessages({
    ...
    [ERROR.SLUG_NOT_ALLOWED]: {
      id: 'Error.SlugNotAllowed',
      defaultMessage: 'This slug is not allowed.',
    },
    [ERROR.SLUG_USED]: {
      id: 'Error.SlugUsed',
      defaultMessage: 'This slug is already used. Please choose another one.',
    },
    [ERROR.INVALID_OTP]: {
      id: 'Error.InvalidOTP',
      defaultMessage: 'The OTP code is invalid or has expired.',
    },
    [ERROR.RATE_LIMIT_EXCEEDED]: {
      id: 'Error.RateLimitExceeded',
    ...
    [ERROR.EMAIL_SEND_ERROR]: {
      id: 'Error.EmailSendError',
      defaultMessage: 'An error occurred while sending the email. Please try again later.',
    },
    [ERROR.USER_ALREADY_EXISTS]: {
      id: 'Error.UserAlreadyExists',
      defaultMessage: 'A user with this email already exists.',
    },
    [ERROR.CAPTCHA_REQUIRED]: {
      id: 'Error.CaptchaRequired',
      defaultMessage: 'Captcha verification is required.',
    },
    [ERROR.OTP_REQUEST_NOT_FOUND]: {
      id: 'Error.OtpRequestNotFound',
      defaultMessage: 'OTP request not found or has expired. Please request a new OTP.',
    },
  }),
  ...transferwiseMsg,
};
...
export const formatErrorMessage = (intl, error, fallback = ERROR.UNKNOWN) => {
  ...
  } else if (error.payload?.code && msg[error.payload.code]) {
    // If the error has a payload with a code, we can use that to format the message
    return intl.formatMessage(msg[error.payload.code], { ...I18nFormatters, ...error.payload });
  }
