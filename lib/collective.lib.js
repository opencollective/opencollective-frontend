...
export const expenseSubmissionAllowed = (collective, user) => {
  const member = user?.memberOf.filter(member => member.collective.id === collective.id);
  const expenses = collective?.expenses?.filter(
    expense => expense.status === expenseStatus.APPROVED && expense.fromCollective.slug === user?.username,
  );
  return member?.length > 0 || !collective?.settings.disableExpenseSubmission || expenses?.length > 0;
};
