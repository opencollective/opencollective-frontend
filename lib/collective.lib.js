...
/** Given a user and a collective see if the user is an admin of other collective under the same host **/
export const isUserAdminOfCollectiveUnderSameHost = (user, account) => {
  const sameHostCollectives = user?.memberOf.filter(obj => obj.collective?.host?.id === account.host.legacyId);
  return sameHostCollectives?.length > 0;
};
