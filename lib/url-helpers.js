...
export const getSettingsRoute = (account, section, LoggedInUserOrHasNewAdmin) => {
  ...
  if (hasNewAdmin) {
    const hrefBase = `/admin-panel?slug=${account.slug}`;
    let linkAsBase = `/${account.slug}/admin`;
    if (parent) {
      if (account.type === 'EVENT') {
        linkAsBase = `/${parent.slug}/events/${linkAsBase}`;
      } else if (account.type === 'PROJECT') {
        linkAsBase = `/${parent.slug}/projects/${linkAsBase}`;
      }
    }

    return !section
      ? { href: hrefBase, linkAs: linkAsBase }
      : { href: `${hrefBase}&section=${section}`, linkAs: `${linkAsBase}/${section}` };
  } else {
    if (account.type === 'EVENT') {
      return { href: `/${parent?.slug || 'collective'}/events/${account.slug}/edit` };
    } else {
      return { href: `/${account.slug}/edit` };
    }
  }
};
