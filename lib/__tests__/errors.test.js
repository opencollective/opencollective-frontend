...
import { createError, ERROR, formatErrorMessage, getErrorFromGraphqlException, isErrorType } from '../errors';
...
describe('lib/errors', () => {
  ...
  describe('isErrorType', () => {
    ...
    it('returns whether error is the same type or not', () => {
      const error = createError(ERROR.BAD_COLLECTIVE_TYPE);
  ...
  describe('getErrorFromGraphqlException', () => {
    ...
    it('returns a proper error from extensions.code', () => {
      expect(
        getErrorFromGraphqlException({ graphQLErrors: [{ extensions: { code: ERROR.BAD_COLLECTIVE_TYPE } }] }).type,
  ...
  describe('', () => {
    ...
    it('Has fallback messages', () => {
      ...
      expect(formatErrorMessage(intl, error)).toBe('An unknown error occurred');
    });
