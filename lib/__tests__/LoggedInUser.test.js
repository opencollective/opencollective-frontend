...
describe('General permissions', () => {
  describe('hasRole', () => {
    it('returns true if user has one of the requested roles', () => {
      ...
      // expect(testUser.hasRole('ADMIN', null)).toBe(false);
    });
  });
  ...
  describe('isHostAdmin', () => {
    it('returns true if user is admin of the host', () => {
      expect(testUser.isHostAdmin(randomCollective)).toBeFalsy(); // should be strictly false, currently returns null
      expect(testUser.isHostAdmin(adminCollective)).toBeFalsy(); // should be strictly false, currently returns null
      expect(testUser.isHostAdmin(hostAdminCollective)).toBe(true);

      // TODO: Should not crash
      // expect(testUser.isHostAdmin(null)).toBeFalsy();
    });
  });
...
describe('Collectives', () => {
  describe('canEditCollective', () => {
    it('returns true if user can edit collective', () => {
      ...
      expect(testUser.canEditCollective({ ...randomCollective, createdByUser: testUser })).toBe(true);
...
describe('Expenses', () => {
  describe('canApproveExpense', () => {
    it('returns true if user can approve/reject expense', () => {
      ...
      expect(testUser.canApproveExpense({ collective: randomCollective, createdByUser: testUser })).toBe(false);
  ...
  describe('canPayExpense', () => {
    it('returns true if user can approve/reject expense', () => {
      ...
      expect(testUser.canPayExpense({ collective: randomCollective, createdByUser: testUser })).toBe(false);
...
describe('Comments', () => {
  describe('canEditComment', () => {
    it('returns true if user can edit comment', () => {
      ...
      expect(testUser.canEditComment({ collective: randomCollective, createdByUser: testUser })).toBe(true);
...
describe('Substripitons', () => {
  describe('canEditSubscription', () => {
    it('returns true if user can edit comment', () => {
      expect(testUser.canEditSubscription({ fromCollective: randomCollective })).toBe(false);
      expect(testUser.canEditSubscription({ fromCollective: backedCollective })).toBe(false);
      expect(testUser.canEditSubscription({ fromCollective: memberCollective })).toBe(false);
      expect(testUser.canEditSubscription({ fromCollective: randomEventCollective })).toBe(false);

      expect(testUser.canEditSubscription({ fromCollective: adminCollective })).toBe(true);
      expect(testUser.canEditSubscription({ fromCollective: adminEventCollective })).toBe(true);
      expect(testUser.canEditSubscription({ fromCollective: { ...randomCollective, createdByUser: testUser } })).toBe(
        true,
      );

      // TODO: Should not crash
      // expect(testUser.canEditSubscription({ fromCollective: null })).toBe(false);
    });
  });
});
