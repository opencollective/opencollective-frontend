...
export function uploadImageWithXHR(file, kind, { onProgress, onSuccess, onFailure, mockImage = false }) {
  ...
  if (mockImage && canUseMockImageUpload()) {
    const mockImage = mockImage === true ? 'https://d.pr/free/i/OlQVIb+' : mockImage;
    return mockImageUpload(mockImage, { onProgress });
  }
...
export function sendInAppSurveyResponse(body) {
  return fetch('/api/in-app-survey', {
    method: 'POST',
    headers: {
      ...addAuthTokenToHeader(),
      Accept: 'application/json',
