...
import { GQLV2_PAYMENT_METHOD_TYPES, PaymentMethodService, PaymentMethodType } from './constants/payment-methods';
...
export const getPaymentMethodIcon = (pm, collective, size) => {
  ...
  } else if (pm.type === PaymentMethodType.GIFT_CARD) {
    return <GiftCard size={size} />;
  } else if (pm.service === PaymentMethodService.PAYPAL || pm.service === GQLV2_PAYMENT_METHOD_TYPES.PAYPAL) {
...
export const paymentMethodTypeHasBalance = type => {
  return [PaymentMethodType.COLLECTIVE, PaymentMethodType.GIFT_CARD, PaymentMethodType.PREPAID].includes(type);
};
...
export const isPaymentMethodDisabled = (pm, totalAmount) => {
  if (isPaymentMethodExpired(pm)) {
    ...
    if (pm.type === PaymentMethodType.GIFT_CARD && balance < MIN_GIFT_CARD_BALANCE) {
      return true;
...
export const getPaymentMethodMetadata = (pm, totalAmount) => {
  ...
  } else if (pm.type === PaymentMethodType.GIFT_CARD && balance < MIN_GIFT_CARD_BALANCE) {
    return (
      <FormattedMessage
  ...
  } else if (pm.service === GQLV2_PAYMENT_METHOD_TYPES.PAYPAL) {
    return pm.name;
  } else {
    return null;
  }
