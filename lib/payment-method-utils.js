...
export const getPaymentMethodIcon = (pm, collective, size) => {
  ...
  const service = pm.service?.toUpperCase();
...
export const paymentMethodTypeHasBalance = type => {
  // TODO(paymentMethodType): remove toUpperCase once migration is over
  return [PAYMENT_METHOD_TYPE.COLLECTIVE, PAYMENT_METHOD_TYPE.GIFTCARD, PAYMENT_METHOD_TYPE.PREPAID].includes(
    type?.toUpperCase(),
  );
};
...
export const isPaymentMethodDisabled = (pm, totalAmount) => {
  if (isPaymentMethodExpired(pm)) {
    ...
    // TODO(paymentMethodType): remove toUpperCase once migration is over
    if (pm.type?.toUpperCase() === PAYMENT_METHOD_TYPE.GIFTCARD && balance < MIN_GIFT_CARD_BALANCE) {
      return true;
