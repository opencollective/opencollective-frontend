import { get, isNil, min, orderBy, partition, uniq } from 'lodash';
...
export const TICKETS_ORDER_KEY = 'collectivePage.ticketsOrder';
...
/**
 * Sort tickets by order keys, and then by endsAt descending.
 */
export const sortTickets = (tickets, orderKeys) => {
  const [sortedTickets, unsortedTickets] = partition(tickets, ticket => orderKeys.includes(ticket.id));
  return [
    ...orderBy(sortedTickets, ticket => orderKeys.findIndex(key => key === ticket.id)),
    ...orderBy(unsortedTickets, ['endsAt', 'id'], ['desc', 'desc']),
  ];
};

export const getCollectiveTicketsOrder = collective => {
  return get(collective.settings, TICKETS_ORDER_KEY, EMPTY_ARRAY);
};

export const sortTiersForCollective = (collective, baseTiers, alwaysIncludeCustomContribution = false) => {
  const orderKeys = get(collective.settings, TIERS_ORDER_KEY, EMPTY_ARRAY);
  const hasCustomContribution =
    alwaysIncludeCustomContribution || !get(collective, 'settings.disableCustomContributions', false);
