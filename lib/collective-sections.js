...
const getSectionsToRemoveForUser = (collective, isAdmin) => {
  ...
  const hasAccessToFeature = (feature, collective) => {
    const status = features[feature];
    return (status === 'ACTIVE' || (status === 'AVAILABLE' && isAdmin)) && !collective.isFrozen;
  };
  ...
  if (!hasAccessToFeature(FEATURES.PROJECTS, collective)) {
    toRemove.add(Sections.PROJECTS);
  }
  if (!hasAccessToFeature(FEATURES.COLLECTIVE_GOALS, collective)) {
  ...
  if (!hasAccessToFeature(FEATURES.EVENTS, collective)) {
    toRemove.add(Sections.EVENTS);
  }
  if (!hasAccessToFeature(FEATURES.UPDATES, collective)) {
    toRemove.add(Sections.UPDATES);
  }
  if (!hasAccessToFeature(FEATURES.CONVERSATIONS, collective)) {
  ...
  if (!hasAccessToFeature(FEATURES.ABOUT, collective)) {
