...
export const filterSectionsByData = (sections, collective, isAdmin, isHostAdmin) => {
  ...
  // New Connect section
  // Don't display if:
  // 1. both Conversations and Updates are disabled
  // 2. only Updates is enabled but the user viewing is a non-admin user
  const hasNewCollectiveNavbar = parseToBoolean(getEnvVar('NEW_COLLECTIVE_NAVBAR'));
  const hasUpdatesButNotConversations =
    !hasFeature(collective, FEATURES.CONVERSATIONS) && hasFeature(collective, FEATURES.UPDATES);
  const hasNeitherUpdatesNorConversations =
    !hasFeature(collective, FEATURES.CONVERSATIONS) && !hasFeature(collective, FEATURES.UPDATES);
  if (!hasNewCollectiveNavbar) {
    toRemove.add(Sections.CONNECT);
  } else {
    if (hasNeitherUpdatesNorConversations) {
      toRemove.add(Sections.CONNECT);
    } else if (hasUpdatesButNotConversations) {
      if (!isAdmin && !isHostAdmin) {
        toRemove.add(Sections.CONNECT);
      }
    }
  }
  // Remove Goals, Conversations, and Update section if enabled but we are using NEW_COLLECTIVE_NAVBAR as it will be displayed in the Budget section
  if (hasNewCollectiveNavbar) {
    if (hasFeature(collective, FEATURES.COLLECTIVE_GOALS)) {
      toRemove.add(Sections.GOALS);
    }
