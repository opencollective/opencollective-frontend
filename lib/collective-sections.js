...
export const getFilteredSectionsForCollective = (collective, isAdmin, isHostAdmin) => {
  let sections;
  if (hasNewCollectiveNavbar) {
    if (get(collective, 'settings.collectivePage.useNewSections')) {
...
export const convertSectionsToNewFormat = sections => {
  ...
  do {
    let section = sectionsToConvert[0];
    if (typeof section === 'string') {
      section = { section, isEnabled: true };
    }
...
export const hasNewNavBar = collective => {
  return get(collective, 'settings.collectivePage.useNewSections');
};
