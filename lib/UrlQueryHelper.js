...
export default class UrlQueryHelper {
  ...
  static Encoders = {
     ...
     * @returns {string}
     */
    amount: (value, isCrypto) => {
      if (isCrypto) {
        return value.toString();
      } else {
        return centsAmountToFloat(value).toString();
      }
    },
    /**
     * @param {boolean} value
  ...
  decode(queryObject) {
    ...
    Object.entries(this.config).forEach(([key, param]) => {
      ...
      const isCrypto = queryObject.paymentFlow === PAYMENT_FLOW.CRYPTO;
      const decoder = UrlQueryHelper.Decoders[fieldType];
      const decodedValue = decoder ? decoder(value, isCrypto) : value;
  ...
  encode(queryObject) {
    ...
    Object.entries(queryObject).forEach(([key, value]) => {
      ...
      const isCrypto = queryObject.paymentFlow === PAYMENT_FLOW.CRYPTO;
      const encodedValue = encoder ? encoder(value, isCrypto) : value;
