...
const StripeOAuthCallbackPage = () => {
  ...
  const [connectStripeAccount] = useMutation<ConnectStripeAccountMutation, ConnectStripeAccountMutationVariables>(
    connectStripeAccountMutation,
  );

  const hasValidParams = Boolean(code && state) && !oauthError;
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        setIsLoading(true);
        const result = await connectStripeAccount({ variables: { code, state } });
        if (!isMounted) {
          return;
        }
        ...
        const redirectUrl = result.data?.connectStripeAccount?.redirectUrl;
  ...
  return (
    <AuthenticatedPage
      noRobots
      title={intl.formatMessage({ defaultMessage: 'Stripe connection', id: 'StripeConnection' })}
    >
        ...
        <div className="my-8 flex h-full flex-col items-center justify-center gap-3 p-2 sm:my-32">
          <div>
            <Alert variant="destructive">
              <AlertCircle size={16} />
              <AlertTitle>
                <FormattedMessage defaultMessage="Stripe connection failed" id="StripeConnectionFailed" />
              </AlertTitle>
              <AlertDescription>
                {apiError || oauthError || (
                  <FormattedMessage
                    defaultMessage="An error occurred while connecting your Stripe account. Please try again."
                    id="StripeConnectionFailedDetails"
                  />
                )}
              </AlertDescription>
            </Alert>
...
export default StripeOAuthCallbackPage;
